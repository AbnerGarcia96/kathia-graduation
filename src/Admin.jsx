...
import { authenticateAdmin } from './firebase.js';
...
export default function Admin() {
  ...
  const handleAdminLogin = async (event) => {
    ...
    const authenticated = await authenticateAdmin(adminEmail.trim(), adminPassword.trim());
    if (authenticated) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-md rounded-[32px] border border-slate-800/60 bg-slate-900/90 p-8 shadow-xl shadow-slate-950/20">
        ...
        <p className="mt-2 text-sm text-slate-300">Enter your Firebase admin credentials to continue.</p>
        ...
        {adminAuthenticated ? (
          <div className="mt-6 rounded-3xl border border-cyan-500/10 bg-slate-950/80 p-6 text-slate-100">
            ...
            <p className="mt-3 text-slate-300">You have successfully authenticated with Firebase credentials.</p>
            <button
